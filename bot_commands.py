...
async def api_post(
    ...
    try:
        ...
        log(
            f"API HTTP error on {endpoint}: {e.response.status_code} - {e.response.text}",
            "ERROR",
        )
...
class Moderation(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send(
                f"❌ Missing: `{error.param.name}` → try `{ctx.prefix}{ctx.command.name} {ctx.command.signature}`"
            )
            ...
            await ctx.send(
                f"❌ I don't have permission to do that: {', '.join(error.missing_permissions)}"
            )
        else:
    ...
    @commands.command(
        name="prefix",
        help="Show or set prefixes. Usage: prefix, prefix add !, prefix remove !, prefix reset",
    )
        ...
        *,
        ...
        if action == "add" and value:
            ...
            await asyncio.to_thread(
                requests.patch,
                f"{SKILLSYNC_API}/observer/guilds/{ctx.guild.id}/prefix",
        ...
        if action == "remove" and value:
            ...
            await ctx.send(
                f"Removed prefix `{value}`. Active: `{'`, `'.join(prefixes)}`"
            )
            return
        ...
        if not value:
            ...
            await ctx.send(
                f"Added prefix `{action}`. Active: `{'`, `'.join(prefixes)}`"
            )
            return
    ...
    @commands.command(
        name="scan", help="Re-scan guild roles, members, and permissions."
    )
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(
        name="timeout", help="Timeout a user. Usage: timeout @user <minutes> [reason]"
    )
    @commands.has_permissions(moderate_members=True)
    async def timeout(
        self, ctx, target: discord.Member, minutes: int, *, reason="No reason given"
    ):
        ok, err = self.can_moderate(ctx, target)
        if not ok:
            await ctx.send(err)
        ...
        try:
            ...
            await ctx.send(
                f"Timed out {target.name} for {minutes} min. Reason: {reason}"
            )
        except discord.Forbidden:
    ...
    @commands.command(
        name="purge",
        aliases=["clear"],
        help="Delete recent messages. Usage: purge <count>",
    )
    ...
    @commands.command(
        name="inrole",
        help='List all members with a given role. Usage: inrole @role / "Role Name" / RoleID',
    )
    @commands.has_permissions(moderate_members=True)
    @commands.guild_only()
    async def inrole(self, ctx, *, name: str):
        # Resolve role: mention > ID > name match
        role = None
        if ctx.message.role_mentions:
            role = ctx.message.role_mentions[0]
        if role is None:
            try:
                role = ctx.guild.get_role(int(name.strip()))
            except ValueError:
                pass
        if role is None:
            exact = discord.utils.get(ctx.guild.roles, name=name.strip())
            if exact:
                role = exact
            else:
                matches = [r for r in ctx.guild.roles if name.lower() in r.name.lower()]
                if len(matches) == 1:
                    role = matches[0]
                elif len(matches) > 1:
                    await ctx.send(
                        f"❌ Multiple roles match '{name}': "
                        + ", ".join(f"`{r.name}`" for r in matches[:5])
                        + ". Be more specific."
                    )
                    return
                else:
                    await ctx.send(f"❌ No role found matching '{name}'.")
                    return

        members_with_role = [m for m in ctx.guild.members if role in m.roles]
        if not members_with_role:
            await ctx.send(f"📭 No cached members have the `{role.name}` role.")
            return

        lines = [
            f"**{role.name}** — {len(members_with_role)} member{'s' if len(members_with_role) != 1 else ''} (cached)"
        ]
        for i, m in enumerate(members_with_role[:50], 1):
            lines.append(f"{i}. {m.mention} (`{m.id}`)")
        if len(members_with_role) > 50:
            lines.append(f"*...and {len(members_with_role) - 50} more*")
        await ctx.send("\n".join(lines))
    ...
    async def job(self, ctx):
        await ctx.send(
            "https://media.discordapp.net/attachments/871441590453956628/1521966733638893779/IMG_6017.png?ex=6a46c17a&is=6a456ffa&hm=7463bb6c12e19300b3917ccce9a1fecfb7dc6eeff64e850fb094d0b140461cb0&=&format=webp&quality=lossless"
        )
    ...
    @commands.command(
        name="avatar",
        aliases=["av", "pfp"],
        help="Show a user's avatar. Usage: avatar @user (defaults to yourself)",
    )
