...
if not API_KEY:
    raise RuntimeError(
        "API_KEY environment variable is not set — all requests would be rejected"
    )
...
def log_action():
    ...
    ok, err = validate_payload(
        data, ["discord_id", "staff_name", "action_type", "guild"]
    )
    if not ok:
    ...
    if worker and points:
        ...
        if action_type == "ban_issued" and discord_id and gid:
            ...
            existing = PendingBan.query.filter_by(
                guild_id=gid, user_id=target_id
            ).first()
            if not existing:
                db.session.add(
                    PendingBan(
            ...
            PendingBan.query.filter_by(
                guild_id=gid, user_id=sanitize_str(data.get("target_id", ""), 50)
            ).delete()
        db.session.commit()
...
def log_warn():
    ...
    worker = (
        Worker.query.filter(Worker.name.ilike(f"%{mod_name}%")).first()
        if mod_name
        else None
    )
...
def confirm_action():
    ...
    if guild_id and target_id:
        deleted = PendingBan.query.filter_by(
...
def log_messages():
    ...
    guild_ids = set(
        sanitize_str(m.get("guild_id", ""), 50) for m in messages if m.get("guild_id")
    )
    content_ok = set()
    if guild_ids:
        rows = (
            GuildInfo.query.with_entities(GuildInfo.guild_id, GuildInfo.store_content)
            .filter(GuildInfo.guild_id.in_(guild_ids))
            .all()
        )
    ...
    for msg in messages:
        ...
        content = (
            sanitize_str(msg.get("content"), 2000)
            if msg.get("content") and gid in content_ok and msg.get("is_public", True)
            else None
        )
...
def user_mention_analytics(discord_id):
    ...
    slowest_reply = (
        MentionRecord.query.filter_by(mentioned_id=discord_id)
        .filter(MentionRecord.reply_time_seconds != None)
        .order_by(MentionRecord.reply_time_seconds.desc())
        .first()
    )
    ...
    return jsonify(
        {
            ...
            "slowest_reply_seconds": round(slowest_reply.reply_time_seconds or 0, 1)
            if slowest_reply
            else None,
        }
    )
...
def user_analytics(discord_id):
    ...
    stats = (
        ...
        .filter(MessageRecord.discord_id == discord_id)
        .first()
    )
    ...
    channels = (
        db.session.query(
            MessageRecord.channel_name, func.count(MessageRecord.id).label("count")
        )
        .filter(MessageRecord.discord_id == discord_id)
...
def all_analytics():
    ...
    top_users = (
        ...
        .group_by(MessageRecord.discord_id, MessageRecord.name)
        .order_by(func.count(MessageRecord.id).desc())
        .limit(10)
        .all()
    )
    ...
    return jsonify(
        {
            ...
            "top_users": [
                {"id": u.discord_id, "name": u.name, "messages": u.count}
                for u in top_users
            ],
...
def receive_guild_scan():
    ...
    existing_members = {
        m.member_id: m for m in GuildMember.query.filter_by(guild_id=guild_id).all()
    }
    scanned_ids = set()
    ...
    return jsonify(
        {
            "message": "Guild scan stored",
            "guild": guild.name,
            "staff": guild.staff_count,
        }
    ), 201
...
def list_guild_members(guild_id):
    ...
    members = query.order_by(
        GuildMember.top_role_position.desc(), GuildMember.name
    ).all()
    return jsonify(
        [
            {
                ...
                "last_seen_online": m.last_seen_online.isoformat()
                if m.last_seen_online
                else None,
                "last_message_at": m.last_message_at.isoformat()
...
def list_all_automod_rules():
    ...
    rules = (
        AutoModRule.query.filter(AutoModRule.alert_channel_id != None)
        .order_by(AutoModRule.guild_id, AutoModRule.name)
        .all()
    )
    return jsonify(
        [
...
def receive_presence():
    ...
    for p in updates:
        ...
        if member:
            ...
            member.last_seen_online = (
                now if p.get("is_online", False) else member.last_seen_online
            )
...
def receive_online_count():
    ...
    for guild_id_str, count in data.items():
        if not isinstance(count, int):
            continue
        guild = GuildInfo.query.filter_by(
            guild_id=sanitize_str(guild_id_str, 50)
        ).first()
        if guild:
...
def detect_anomalies_for_user(discord_id, name=None):
    ...
    all_msgs = (
        MessageRecord.query.filter_by(discord_id=discord_id)
    ...
    baseline_std_len = (
        math.sqrt(sum((x - baseline_avg_len) ** 2 for x in lengths) / len(lengths))
        if len(lengths) > 1
        else 0
    )
    ...
    if baseline_msgs:
        days_span = max(
            1, (baseline_msgs[-1].created_at - baseline_msgs[0].created_at).days or 1
        )
...
def scan_anomalies():
    ...
    now = datetime.utcnow()
    discord_ids = (
        db.session.query(MessageRecord.discord_id, MessageRecord.name).distinct().all()
    )

    total_flagged = 0
    # Track newly created anomalies for Slack notifications
    new_anomaly_records = []
    for discord_id, name in discord_ids:
        ...
        for a in anomalies:
            existing = (
                ...
                .filter(BehavioralAnomaly.detected_at > now - timedelta(hours=12))
                .first()
            )
            if not existing:
    ...
    # Notify Slack for new anomalies belonging to linked workspace workers
    try:
        from database import WorkerIdentity
        from services.slack import notify_anomaly_detected

        for rec in new_anomaly_records:
            ident = WorkerIdentity.query.filter_by(
                discord_id=rec["discord_id"], is_active=True
            ).first()
            if ident and ident.worker_id and ident.consent_community_prior:
                notify_anomaly_detected(
                    worker_name=ident.display_name or rec["name"] or rec["discord_id"],
...
def list_anomalies():
    ...
    anomalies = (
        BehavioralAnomaly.query.filter_by(cleared_at=None)
        .filter(BehavioralAnomaly.severity >= min_sev)
...
def user_anomalies(discord_id):
    ...
    anomalies = (
        BehavioralAnomaly.query.filter_by(discord_id=discord_id, cleared_at=None)
        .order_by(BehavioralAnomaly.severity.desc())
        .all()
    )
    return jsonify(
...
def log_voice_activity():
    ...
    for s in sessions:
        entry = VoiceActivity(
            ...
            if s.get("hour_of_day") is not None
            else None,
            day_of_week=int(s["day_of_week"])
            if s.get("day_of_week") is not None
            else None,
            joined_at=datetime.fromisoformat(s["joined_at"])
            if s.get("joined_at")
            else None,
...
def scan_burnout_risks():
    ...
    for w in workers:
        ...
        voice_recent = (
            ...
            .filter(
                VoiceActivity.discord_id == did, VoiceActivity.created_at >= cutoff_14
            )
            .scalar()
            or 0
        )
        ...
        if reversals_prior > 0 and reversals_recent > reversals_prior:
            reversal_risk = min(
                1.0, (reversals_recent - reversals_prior) / reversals_prior
            )
        ...
        if has_volume_anomaly:
            signals.append("volume_volatility")
        ...
        if existing:
            ...
            br = BurnoutRisk(
                ...
                reversal_rate=reversal_risk,
                voice_creep=voice_creep,
    ...
    return jsonify(
        {"scanned": len(results), "flagged": len(flagged), "results": results}
    )
...
def ml_scan_burnout():
    ...
    from database import BurnoutRisk, Worker
    ...
    for fb in flagged:
        ...
        if existing:
            ...
            br = BurnoutRisk(
                ...
                discord_id=fb["discord_id"],
                name=fb["name"],
                score=fb["burnout_score"],
                signals=signals_str,
            )
            db.session.add(br)
        updated += 1
...
def ml_growth_anomalies(guild_id):
    ...
    return jsonify(
        {"guild_id": guild_id, "anomalies": anomalies, "count": len(anomalies)}
    )
...
def log_role_change():
    ...
    if modifier_id:
        ...
        if modifier:
            if category == "promotion":
                points_awarded = 5
                reason = (
                    f"Promoted {data['member_name']} to staff ({data['role_name']})"
                )
            elif category == "demotion":
                points_awarded = 3
                reason = (
                    f"Demoted {data['member_name']} from staff ({data['role_name']})"
                )
            elif category == "retirement":
                points_awarded = 2
                reason = (
                    f"Retired {data['member_name']} from staff ({data['role_name']})"
                )
            elif category == "other":
                points_awarded = 1
    ...
    db.session.commit()
    return jsonify(
        {"status": "logged", "id": record.id, "points_awarded": points_awarded}
    )
