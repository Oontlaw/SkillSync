...
# Use a simple path under project root to avoid Windows \\?\ long-path prefix
TEST_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test.db")
# Ensure forward slashes for SQLite URI on Windows
TEST_DB_URI = TEST_DB.replace("\\", "/")
os.environ.setdefault("SECRET_KEY", "test-secret-key")
os.environ.setdefault("API_KEY", "test-api-key")
os.environ["DATABASE_URL"] = f"sqlite:///{TEST_DB_URI}"
...
def login_discord(client, guilds):
    with client.session_transaction() as session:
        ...
        session["accessible_guilds"] = [
            {"id": str(guild_id), "name": f"Guild {guild_id}"} for guild_id in guilds
        ]
