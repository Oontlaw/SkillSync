...
def _build_features(guild_id, days=90):
    ...
    for i, ts in enumerate(sorted_ts):
        ...
        lag_leaves = sum(
            hourly[t]["leave"] for t in sorted_ts if window_start <= t < ts
        )
...
def train(guild_id, days=90):
    ...
    if X is None or len(y_join) < 48:
        return {
            "status": "skipped",
            "reason": f"Only {len(y_join) if y_join is not None else 0} hourly data points",
        }
...
def predict_next_7d(guild_id):
    ...
    # Compute REAL lag features from DB (last 24h of actual join/leave counts)
    lag_cutoff = now - timedelta(hours=24)
    recent_rows = (
        MemberJoinLeave.query.filter(
            MemberJoinLeave.guild_id == guild_id,
            MemberJoinLeave.created_at >= lag_cutoff,
        )
        .with_entities(MemberJoinLeave.event_type)
        .all()
    )

    lag_joins_24h = sum(1 for r in recent_rows if r.event_type == "join")
    lag_leaves_24h = sum(1 for r in recent_rows if r.event_type == "leave")
    ...
    for offset in range(168):
        ...
        # Use real lag for first 24h, decay toward mean after that
        decay = max(0.0, 1.0 - offset / 24.0)
        effective_lag_joins = lag_joins_24h * decay
        effective_lag_leaves = lag_leaves_24h * decay
        ...
        preds.append(
            {
                ...
                "dow": dow,
                "offset_hours": offset,
                "predicted_joins": pred_join,
                "predicted_leaves": pred_leave,
            }
        )
    ...
    return {
        ...
        "lag_joins_used": lag_joins_24h,
        "lag_leaves_used": lag_leaves_24h,
    }
...
def detect_anomalous_growth(guild_id, days=30, z_threshold=2.5):
    ...
    if len(join_vals) >= 7:
        ...
        l_mean, l_std = (
            statistics.mean(leave_vals),
            max(statistics.stdev(leave_vals), 1),
        )
